...
def resolve_checkpoint(path: str | None, repo: str, filename: str) -> str:
    # もしpathが指定されていればそれを返し、そうでなければHugging Face Hubからダウンロードする
...
def export_hf_dataset(
    ...
    audio_col = args.hf_audio_column # datasetの音声データが入っているカラム名
    key_col = args.hf_key_column     # datasetの音声データのindexのカラム名
...
def main() -> None:
    ...
    # clense_multilingual_librispeechの実装はcleanse_webdataset.pyに依存しているため、同じリポジトリ内にあることを前提とする
    ...
    # expanduser(): ~を含むパスを展開して絶対パスに変換, resolve(): シンボリックリンクを解決して絶対パスに変換
    output_dir = Path(args.output_dir).expanduser().resolve()

    # 特徴抽出器&vocoderのパスを解決。ユーザーが明示的に指定した場合はそれを使用し、そうでない場合はHugging Face Hubからダウンロードする
    ...
    # --hf-output-dirが指定された場合はCLI経由ではなく直接Hugging Faceデータセットを出力する
    if args.hf_output_dir:
        # dry_runが指定された場合は分離される予定のHugging Faceデータセットを出力する
        if args.dry_run:
            ...
            return
        # --hf-streamingはCLI経由での出力にのみ関連するため、HFデータセット出力と組み合わせることはできない
        if args.hf_streaming:
